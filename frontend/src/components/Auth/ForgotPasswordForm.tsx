...
type Step = 'email' | 'reset' | 'setup' | 'success'
...
export function ForgotPasswordForm() {
  ...
  const handleSetupSubmit = async (e: FormEvent) => {
    ...
    try {
      await authService.firstTimeReset({
        email,
        recovery_pin: newPin,
        recovery_pin_confirm: confirmPin,
        new_password: newPassword,
        new_password_confirm: confirmPassword,
      })
      setStep('success')
  ...
  const renderStepIndicator = () => {
    const steps = [
      { key: 'email', label: '1' },
      { key: step === 'setup' ? 'setup' : 'reset', label: '2' },
    ]
  ...
  const getSubtitle = () => {
    if (step === 'email') return 'Inserisci la tua email'
    if (step === 'setup') return 'Configura il PIN di recupero e imposta la nuova password'
    return 'Inserisci il PIN e la nuova password'
  }
  ...
  return (
    ...
    >
      ...
      {/* Step 2a: Reset Password with existing PIN */}
      {step === 'reset' && (
      ...
      {/* Step 2b: First-time setup (no recovery configured) */}
      {step === 'setup' && (
        <form onSubmit={handleSetupSubmit} className="space-y-5">
          <div className="bg-amber-50 border border-amber-200 text-amber-700 px-4 py-3 rounded-xl text-sm">
            Il tuo account non ha un PIN di recupero configurato. Configuralo ora per poter cambiare la password.
          </div>

          <div>
            <label htmlFor="newPin" className="block text-sm font-medium text-gray-700 mb-1.5">
              Crea PIN di Recupero (6 cifre)
            </label>
            <div className="relative">
              <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                <svg className="h-5 w-5 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
